...
class LoRALinear(eqx.Module):
    ...
    def __init__(self, in_features, out_features, rank, key):
        # Initialize the primary weight matrix
        w_key, a_key, b_key = jax.random.split(key, 3)
        self.W = jax.random.normal(w_key, (out_features, in_features))
        # Initialize low-rank matrices A and B
        self.A = jax.random.normal(a_key, (rank, in_features)) * 0.1  # Initialize with small values
        self.B = jax.random.normal(b_key, (out_features, rank)) * 0.1  # Initialize with small values

    def __call__(self, x):
        # Compute the effective weight as W_eff = W + A @ B
        effective_weight = self.W + jnp.dot(self.B, self.A)
        return jnp.dot(effective_weight, x)
...
class Attention(eqx.Module):
    ...
    def __init__(self, args, key, dtype=jnp.bfloat16):
        ...
        self.wq = LoRALinear(args.dim, args.n_heads * args.head_dim, rank, key1, dtype=dtype)
        self.wk = LoRALinear(args.dim, args.n_kv_heads * args.head_dim, rank, key2, dtype=dtype)
        self.wv = LoRALinear(args.dim, args.n_kv_heads * args.head_dim, rank, key3, dtype=dtype)
        self.wo = LoRALinear(args.n_heads * args.head_dim, args.dim, rank, key4, dtype=dtype)
