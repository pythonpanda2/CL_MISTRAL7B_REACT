...
def get_argparse():
    ...
    parser.add_argument("-rf","--replay_frac", type=float, default=1.0,
            help="Fraction (0–1] of the aggregate replay buffer to keep")
...
def main():
    ...
    replay_frac = max(0.0, min(1.0, replay_frac)) # clip to (0,1]
    print("\nReplay Fraction:", replay_frac)
    ...
    for i, key in enumerate(task_keys ) :
        ...
        if i > 0:
            ...
            # --- subsample --------------------------------------
            if replay_frac < 1.0:
                n_total   = exp_rxn_all.shape[0]
                n_choose  = max(1, int(replay_frac * n_total))
                rng       = np.random.default_rng(seed + i)      # reproducible per task
                idx       = rng.choice(n_total, n_choose, replace=False)

                exp_rxn_all    = exp_rxn_all[idx]
                exp_yields_all = exp_yields_all[idx]
